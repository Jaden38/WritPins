...
const Home: React.FC = () => {
  ...
  const handleAddPin = async (title: string, text: string, tags: string[], source: string, isPublic: boolean) => {
    if (user) {
      const newPin: Pin = { title, text, userId: user.uid, tags, source, createdAt: new Date().toISOString(), public: isPublic };
      await addPin(newPin);
      fetchPins(user.uid);
    }
  };
  ...
  const handlePinClick = (id: string) => {
    history.push(`/pins/${id}?fromPublic=false`);
  };
